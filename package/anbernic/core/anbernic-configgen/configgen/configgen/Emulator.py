...
class Emulator():
    ...
    def __init__(self, name, rom):
        ...
        self.config = Emulator.get_system_config(self.name, "/usr/share/anbernic/configgen/configgen-defaults.yml", "/usr/share/anbernic/configgen/configgen-defaults-arch.yml")
        ...
        # load configuration from anbernic.conf
        recalSettings = UnixSettings(anbernicFiles.anbernicConf)
        ...
        if "shaderset" in self.config and self.config["shaderset"] != "none":
            self.renderconfig = Emulator.get_generic_config(self.name, "/usr/share/anbernic/shaders/configs/" + self.config["shaderset"] + "/rendering-defaults.yml", "/usr/share/anbernic/shaders/configs/" + self.config["shaderset"] + "/rendering-defaults-arch.yml")
        if "shaderset" not in self.config: # auto
            self.renderconfig = Emulator.get_generic_config(self.name, "/usr/share/anbernic/shaders/configs/rendering-defaults.yml", "/usr/share/anbernic/shaders/configs/rendering-defaults-arch.yml")

        # for compatibility with earlier Anbernic versions, let's keep -renderer
    ...
    def get_system_config(system, defaultyml, defaultarchyml):
        ...
        # it is flat in the anbernic.conf which is easier for the end user, but i prefer not flat in the yml files
    ...
    def updateFromESSettings(self):
        try:
            esConfig = ET.parse(anbernicFiles.esSettings)
