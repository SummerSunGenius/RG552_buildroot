...
class Pcsx2Generator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        ...
        # config files
        configureReg(anbernicFiles.pcsx2ConfigDir)
        configureUI(anbernicFiles.pcsx2ConfigDir, anbernicFiles.BIOS, system.config, gameResolution)
        configureVM(anbernicFiles.pcsx2ConfigDir, system)
        configureGFX(anbernicFiles.pcsx2ConfigDir, system)
        configureAudio(anbernicFiles.pcsx2ConfigDir)

        if isAVX2:
            commandArray = [anbernicFiles.anbernicBins['pcsx2_avx2'], rom]
        else:
            commandArray = [anbernicFiles.anbernicBins['pcsx2'], rom]
        ...
        # plugins
        real_pluginsDir = anbernicFiles.pcsx2PluginsDir
        if isAVX2:
            real_pluginsDir = anbernicFiles.pcsx2Avx2PluginsDir
        ...
        # arch
        arch = "x86"
        with open('/usr/share/anbernic/anbernic.arch', 'r') as content_file:
            arch = content_file.read()

        if arch == "x86":
            return Command.Command(array=commandArray, env={"XDG_CONFIG_HOME":anbernicFiles.CONF})
        else:
            return Command.Command(array=commandArray, env={"XDG_CONFIG_HOME":anbernicFiles.CONF, "LD_LIBRARY_PATH": "/lib32", "LIBGL_DRIVERS_PATH": "/lib32/dri"})

def getGfxRatioFromConfig(config, gameResolution):
    # 2: 4:3 ; 1: 16:9
    if "ratio" in config:
        if config["ratio"] == "3/5" or (config["ratio"] == "auto" and gameResolution["width"] / float(gameResolution["height"]) < (16.0 / 9.0) - 0.1): # let a marge):
            return "5:3"
	elif config["ratio"] == "4/3":
            return "4:3"
        else:
            return "16:9"
    return "3:5"
