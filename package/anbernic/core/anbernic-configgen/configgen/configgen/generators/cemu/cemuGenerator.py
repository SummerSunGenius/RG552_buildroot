...
class CemuGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        if not path.isdir(anbernicFiles.BIOS + "/cemu"):
            os.mkdir(anbernicFiles.BIOS + "/cemu")
        if not path.isdir(anbernicFiles.CONF + "/cemu"):
            os.mkdir(anbernicFiles.CONF + "/cemu")

        for folder in ["shaderCache", "controllerProfiles", "gameProfiles", "graphicPacks"]:
            if not path.isdir(anbernicFiles.CONF + "/cemu/" + folder):
                os.mkdir(anbernicFiles.CONF + "/cemu/" + folder)

        if not path.isdir(anbernicFiles.SAVES + "/cemu"):
            os.mkdir(anbernicFiles.SAVES + "/cemu")

        CemuGenerator.CemuConfig(anbernicFiles.CONF + "/cemu/settings.xml")
        # copy the file from where cemu reads it
        copyfile(anbernicFiles.CONF + "/cemu/settings.xml", "/usr/cemu/settings.xml")

        cemuControllers.generateControllerConfig(system, playersControllers, rom)

        commandArray = ["wine64", "/usr/cemu/Cemu.exe", "-g", "z:" + rom, "-m", "z:" + anbernicFiles.SAVES + "/cemu", "-f"]
        return Command.Command(
            array=commandArray,
            env={
                "WINEPREFIX": anbernicFiles.SAVES + "/cemu",
                "vblank_mode": "0",
                "mesa_glthread": "true",
