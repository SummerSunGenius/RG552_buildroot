...
def generateControllerConfig(system, playersControllers, rom):
    ...
    if not path.isdir(anbernicFiles.CONF + "/cemu/controllerProfiles"):
        os.mkdir(anbernicFiles.CONF + "/cemu/controllerProfiles")

    if not path.isdir(anbernicFiles.CONF + "/evmapy"):
        os.mkdir(anbernicFiles.CONF + "/evmapy")

    #purge old controller files
    for counter in range(0,8):
        configFileName = "{}/{}".format(anbernicFiles.CONF + "/cemu/controllerProfiles/", "controller" + str(counter) +".txt")
        if os.path.isfile(configFileName):
            os.remove(configFileName)

    #Create the evmapy configFile

    configFileName = "{}/{}".format(anbernicFiles.CONF + "/evmapy/","wiiu.keys")
    if os.path.isfile(configFileName):
        os.remove(configFileName)
    ...
    with open(anbernicFiles.CONF + "/evmapy/wiiu.keys", 'w') as outfile:
    ...
    for playercontroller, pad in sorted(playersControllers.items()):
        ...
        configFileName = "{}/{}".format(anbernicFiles.CONF + "/cemu/controllerProfiles/", "controller" + str(nplayer) + ".txt")
                # save dolphin.ini
