...
class FlycastGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        ...
        if os.path.exists(anbernicFiles.flycastConfig):
            try:
                Config.read(anbernicFiles.flycastConfig)
            except:
        ...
        # custom : allow the user to configure directly emu.cfg via anbernic.conf via lines like : dreamcast.flycast.section.option=value
        ...
        if not os.path.exists(os.path.dirname(anbernicFiles.flycastConfig)):
            os.makedirs(os.path.dirname(anbernicFiles.flycastConfig))
        with open(anbernicFiles.flycastConfig, 'w+') as cfgfile:
            Config.write(cfgfile)        
            cfgfile.close()
            
        # internal config
        # vmuA1
        if not isfile(anbernicFiles.flycastVMUA1):
            if not isdir(dirname(anbernicFiles.flycastVMUA1)):
                os.mkdir(dirname(anbernicFiles.flycastVMUA1))
            copyfile(anbernicFiles.flycastVMUBlank, anbernicFiles.flycastVMUA1)
        # vmuA2
        if not isfile(anbernicFiles.flycastVMUA2):
            if not isdir(dirname(anbernicFiles.flycastVMUA2)):
                os.mkdir(dirname(anbernicFiles.flycastVMUA2))
            copyfile(anbernicFiles.flycastVMUBlank, anbernicFiles.flycastVMUA2)
        ...
        return Command.Command(array=commandArray, env={"XDG_CONFIG_HOME":anbernicFiles.CONF, "XDG_DATA_HOME":anbernicFiles.flycastSaves, "XDG_DATA_DIRS":anbernicFiles.flycastBios})
