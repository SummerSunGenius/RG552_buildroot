...
class Rpcs3Generator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        ...
        if not os.path.exists(os.path.dirname(anbernicFiles.rpcs3CurrentConfig)):
            os.makedirs(os.path.dirname(anbernicFiles.rpcs3CurrentConfig))
        ...
        if os.path.exists(anbernicFiles.rpcs3CurrentConfig):
            rpcsCurrentSettings.read(anbernicFiles.rpcs3CurrentConfig)
        ...
        with open(anbernicFiles.rpcs3CurrentConfig, 'w') as configfile:
            rpcsCurrentSettings.write(configfile)
        
        if not os.path.exists(os.path.dirname(anbernicFiles.rpcs3config)):
            os.makedirs(os.path.dirname(anbernicFiles.rpcs3config))    

        # Generate a default config if it doesn't exist otherwise just open the existing
        if os.path.isfile(anbernicFiles.rpcs3config):
            with open(anbernicFiles.rpcs3config, 'r') as stream:
        ...
        with open(anbernicFiles.rpcs3config, 'w') as file:
            documents = yaml.safe_dump(rpcs3ymlconfig, file, default_flow_style=False)

        romBasename = path.basename(rom)
        romName = rom + '/PS3_GAME/USRDIR/EBOOT.BIN'
        commandArray = [anbernicFiles.anbernicBins[system.config['emulator']], romName]

        if system.isOptSet("gui") and system.getOptBoolean("gui") == False:
            commandArray.append("--no-gui")

        return Command.Command(array=commandArray, env={"XDG_CONFIG_HOME":anbernicFiles.CONF, "XDG_CACHE_HOME":anbernicFiles.SAVES, "QT_QPA_PLATFORM":"xcb"})
