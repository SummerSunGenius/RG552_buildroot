...
def generateConfigInputYml(controllers):
    configFileName = "{}/{}".format(anbernicFiles.CONF, "rpcs3/config_input.yml")
    if not path.isdir(anbernicFiles.CONF + "/rpcs3"):
        os.makedirs(anbernicFiles.CONF + "/rpcs3")
...
def generateInputConfigs(controllers):
    if not path.isdir(anbernicFiles.CONF + "/rpcs3/InputConfigs/Evdev"):
        os.makedirs(anbernicFiles.CONF + "/rpcs3/InputConfigs/Evdev")
    nplayer = 1
    for controller, pad in sorted(controllers.items()):
        if nplayer <= 7:
            configFileName = "{}/{}/pad{}.yml".format(anbernicFiles.CONF, "rpcs3/InputConfigs/Evdev", nplayer)
            f = codecs.open(configFileName, "w", encoding="utf_8_sig")
