...
class AmiberryGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        retroconfig = UnixSettings(anbernicFiles.amiberryRetroarchCustom, separator=' ')
        if not os.path.exists(dirname(anbernicFiles.amiberryRetroarchCustom)):
            os.makedirs(dirname(anbernicFiles.amiberryRetroarchCustom))
        
        romType = self.getRomType(rom)
        eslog.log("romType"+romType)
        if romType != 'UNKNOWN' :           
            commandArray = [ anbernicFiles.anbernicBins[system.config['emulator']], "-G" ]
            if romType != 'WHDL' :
            ...
            if not os.path.exists(anbernicFiles.amiberryRetroarchInputsDir):
                os.makedirs(anbernicFiles.amiberryRetroarchInputsDir)
            nplayer = 1
            for playercontroller, pad in sorted(playersControllers.items()):
                ...
                playerInputFilename = anbernicFiles.amiberryRetroarchInputsDir + "/" + padfilename + ".cfg"
                with open(anbernicFiles.amiberryRetroarchCustom) as infile, open(playerInputFilename, 'w') as outfile:
                    for line in infile:
