...
ratioIndexes = ["4/3", "16/9", "16/10", "16/15", "21/9", "1/1", "2/1", "3/2", "3/4", "3/5", "4/4", "5/4", "6/5", "7/9", "8/3",
...
def createLibretroConfig(system, controllers, rom, bezel, gameResolution):
    ...
    libretroOptions.generateCoreSettings(anbernicFiles.retroarchCoreCustom, system)

    # Create/update hatari.cfg
    if system.name == 'atarist':
        libretroOptions.generateHatariConf(anbernicFiles.hatariConf)
    ...
    if 'shader' in renderConfig and renderConfig['shader'] != None:
        ...
        if os.path.exists("/userdata/shaders/" + shaderFilename):
            ...
            retroarchConfig['video_shader_dir'] = "/usr/share/anbernic/shaders"
    else:
        retroarchConfig['video_shader_enable'] = 'false'

    retroarchConfig['aspect_ratio_index'] = '24' # reset in case config was changed (or for overlays)
    if defined('ratio', systemConfig):
        if systemConfig['ratio'] in ratioIndexes:
            ...
            retroarchConfig['aspect_ratio_index'] = '24'
    ...
    retroarchConfig['savestate_directory'] = anbernicFiles.savesDir + system.name
    retroarchConfig['savefile_directory'] = anbernicFiles.savesDir + system.name
    ...
    if isLowResolution(gameResolution):
        ...
        retroarchConfig['menu_driver'] = 'xmb'
        retroarchConfig['xmb_font'] = '"/usr/share/libretro/assets/xmb/monochrome/font.ttf"'
        retroarchConfig['video_font_path'] = '"/usr/share/libretro/assets/xmb/monochrome/font.ttf"'
    ...
    if system.isOptSet('ai_service_enabled') and system.getOptBoolean('ai_service_enabled') == True:
        ...
        if system.isOptSet('ai_service_url') and system.config['ai_service_url']:
            ...
            retroarchConfig['ai_service_url'] = 'http://ztranslate.net/service?api_key=ANBERNIC&mode=Fast&output=png&target_lang='+chosen_lang
    ...
    hdmi_check_file = open("/sys/devices/platform/singleadc-joypad/hdst")                        
    if hdmi_check_file.readline()=="connected\n":
    ...
    # custom : allow the user to configure directly retroarch.cfg via anbernic.conf via lines like : snes.retroarch.menu_driver=rgui
...
def writeBezelConfig(bezel, retroarchConfig, systemName, rom, gameResolution, bezel_stretch):
    ...
    overlay_cfg_file  = anbernicFiles.overlayConfigFile
    ...
    overlay_info_file = anbernicFiles.overlayUser + "/" + bezel + "/games/" + systemName + "/" + romBase + ".info"
    overlay_png_file  = anbernicFiles.overlayUser + "/" + bezel + "/games/" + systemName + "/" + romBase + ".png"
    bezel_game = True
    if not os.path.exists(overlay_png_file):
        overlay_info_file = anbernicFiles.overlaySystem + "/" + bezel + "/games/" + systemName + "/" + romBase + ".info"
        overlay_png_file  = anbernicFiles.overlaySystem + "/" + bezel + "/games/" + systemName + "/" + romBase + ".png"
        bezel_game = True
        if not os.path.exists(overlay_png_file):
            overlay_info_file = anbernicFiles.overlayUser + "/" + bezel + "/games/" + romBase + ".info"
            overlay_png_file  = anbernicFiles.overlayUser + "/" + bezel + "/games/" + romBase + ".png"
            bezel_game = True
            if not os.path.exists(overlay_png_file):
                overlay_info_file = anbernicFiles.overlaySystem + "/" + bezel + "/games/" + romBase + ".info"
                overlay_png_file  = anbernicFiles.overlaySystem + "/" + bezel + "/games/" + romBase + ".png"
                bezel_game = True
                if not os.path.exists(overlay_png_file):
                    overlay_info_file = anbernicFiles.overlayUser + "/" + bezel + "/systems/" + systemName + ".info"
                    overlay_png_file  = anbernicFiles.overlayUser + "/" + bezel + "/systems/" + systemName + ".png"
                    bezel_game = False
                    if not os.path.exists(overlay_png_file):
                        overlay_info_file = anbernicFiles.overlaySystem + "/" + bezel + "/systems/" + systemName + ".info"
                        overlay_png_file  = anbernicFiles.overlaySystem + "/" + bezel + "/systems/" + systemName + ".png"
                        bezel_game = False
                        if not os.path.exists(overlay_png_file):
                            overlay_info_file = anbernicFiles.overlayUser + "/" + bezel + "/default.info"
                            overlay_png_file  = anbernicFiles.overlayUser + "/" + bezel + "/default.png"
                            bezel_game = True
                            if not os.path.exists(overlay_png_file):
                                overlay_info_file = anbernicFiles.overlaySystem + "/" + bezel + "/default.info"
                                overlay_png_file  = anbernicFiles.overlaySystem + "/" + bezel + "/default.png"
                                bezel_game = True
                                if not os.path.exists(overlay_png_file):
                                    return
