...
def generateCoreSettings(retroarchCore, system):
    ...
    if (system.config['core'] == 'citra'):
        if not os.path.exists(anbernicFiles.CONF + "/retroarch/3ds.cfg"):
            f = open(anbernicFiles.CONF + "/retroarch/3ds.cfg", "w")
    ...
    if (system.config['core'] == 'parallel_n64'):
        coreSettings.save('parallel-n64-gfxplugin', '"rice"')

    if (system.config['core'] == 'desmume'):
        coreSettings.save('desmume_num_cores', '"4"')
        coreSettings.save('desmume_frameskip', '"1"')
        coreSettings.save('desmume_screens_layout', '"left/right"')
    ...
    # custom : allow the user to configure directly retroarchcore.cfg via anbernic.conf via lines like : snes.retroarchcore.opt=val
