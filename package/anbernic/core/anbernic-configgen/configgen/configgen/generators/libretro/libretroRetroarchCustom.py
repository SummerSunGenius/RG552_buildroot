...
def generateRetroarchCustom():
    # retroarchcustom.cfg
    if not os.path.exists(os.path.dirname(anbernicFiles.retroarchCustom)):
        os.makedirs(os.path.dirname(anbernicFiles.retroarchCustom))

    try:
        retroarchSettings = UnixSettings(anbernicFiles.retroarchCustom, separator=' ')
    except UnicodeError:
        os.remove(anbernicFiles.retroarchCustom)
        retroarchSettings = UnixSettings(anbernicFiles.retroarchCustom, separator=' ')

    # Use Interface
    retroarchSettings.save('menu_driver',                       '"rgui"')
    ...
    retroarchSettings.save('menu_swap_ok_cancel_buttons',       '"false"')
...
def generateRetroarchCustomPathes(retroarchSettings):
    ...
    retroarchSettings.save('cheat_database_path',           '"/usr/share/anbernic/datainit/cheats/cht/"')
    ...
    retroarchSettings.save('video_shader_dir',              '"/usr/share/anbernic/shaders/"')
