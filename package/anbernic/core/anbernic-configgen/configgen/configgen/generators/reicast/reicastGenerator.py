...
class ReicastGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        ...
        if os.path.exists(anbernicFiles.reicastConfig):
            try:
                Config.read(anbernicFiles.reicastConfig)
            except:
        ...
        # custom : allow the user to configure directly emu.cfg via anbernic.conf via lines like : dreamcast.reicast.section.option=value
        ...
        if not os.path.exists(os.path.dirname(anbernicFiles.reicastConfig)):
            os.makedirs(os.path.dirname(anbernicFiles.reicastConfig))
        with open(anbernicFiles.reicastConfig, 'w+') as cfgfile:
            Config.write(cfgfile)        
            cfgfile.close()
            
        # internal config
        # vmuA1
        if not isfile(anbernicFiles.reicastVMUA1):
            if not isdir(dirname(anbernicFiles.reicastVMUA1)):
                os.mkdir(dirname(anbernicFiles.reicastVMUA1))
            copyfile(anbernicFiles.reicastVMUBlank, anbernicFiles.reicastVMUA1)
        # vmuA2
        if not isfile(anbernicFiles.reicastVMUA2):
            if not isdir(dirname(anbernicFiles.reicastVMUA2)):
                os.mkdir(dirname(anbernicFiles.reicastVMUA2))
            copyfile(anbernicFiles.reicastVMUBlank, anbernicFiles.reicastVMUA2)
        ...
        return Command.Command(array=commandArray, env={"XDG_CONFIG_HOME":anbernicFiles.CONF, "XDG_DATA_HOME":anbernicFiles.reicastSaves, "XDG_DATA_DIRS":anbernicFiles.reicastBios})
