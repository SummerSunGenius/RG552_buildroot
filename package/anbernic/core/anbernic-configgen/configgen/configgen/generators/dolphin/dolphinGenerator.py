...
class DolphinGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        if not os.path.exists(os.path.dirname(anbernicFiles.dolphinIni)):
            os.makedirs(os.path.dirname(anbernicFiles.dolphinIni))

        # dir required for saves
        if not os.path.exists(anbernicFiles.dolphinData + "/StateSaves"):
            os.makedirs(anbernicFiles.dolphinData + "/StateSaves")
        ...
        if os.path.exists(anbernicFiles.dolphinIni):
            dolphinSettings.read(anbernicFiles.dolphinIni)
        ...
        # save dolphin.ini
        with open(anbernicFiles.dolphinIni, 'w') as configfile:
        ...
        dolphinGFXSettings.read(anbernicFiles.dolphinGfxIni)
		...
		# save gfx.ini
        with open(anbernicFiles.dolphinGfxIni, 'w') as configfile:
            dolphinGFXSettings.write(configfile)

        # update SYSCONF
        try:
            dolphinSYSCONF.update(system.config, anbernicFiles.dolphinSYSCONF, gameResolution)
        except Exception:
        ...
        return Command.Command(array=commandArray, env={"XDG_CONFIG_HOME":anbernicFiles.CONF, "XDG_DATA_HOME":anbernicFiles.SAVES, "QT_QPA_PLATFORM":"xcb"})
