...
function powerdevice_dialog()
{
    ...
    currentswitch=$(anbernic-settings --command load --key system.power.switch)
    ...
    cmd=(dialog --backtitle "ANBERNIC Power Switch Selection Toolset" \
...
function retroflag_start()
{
    ...
    case $(cat /usr/share/anbernic/anbernic.arch) in
        rpi4)
...
function argonone_start()
{
    ...
    # Yes, with kernel 5.4.51 on Anbernic-29, this is required
...
case "$CONFVALUE" in
    ...
    "DIALOG")
        ...
        anbernic-settings --command write --key system.power.switch --value "$switch"
        [[ $? -eq 0 ]] && info_msg="No error! Everything went okay!" || info_msg="An error occurred!"
        dialog --backtitle "ANBERNIC Power Switch Selection Toolkit" \
               --title " STATUS OF NEW VALUE " \
               --msgbox "${info_msg}\n\n$(anbernic-settings status system.power.switch)" 0 0
    ;;
    --HELP|*)
