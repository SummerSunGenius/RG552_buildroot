...
@lbrpdx - Modified to run on Anbernic

Make sure to add in /boot/config.txt:
 dtparam=i2c_arm=on
 dtparam=i2c-1=on

(it should be installed by Anbernic when you enable 
    ...
    while True:
        ...
        if pulsetime >=2 and pulsetime <=3:
            if FORCE_REBOOT == 1:
                ...
                os.system("/usr/bin/anbernic-es-swissknife --reboot" )
            else:
                os.system("/usr/bin/anbernic-es-swissknife --emukill" )
            ...
            os.system("/usr/bin/anbernic-es-swissknife --shutdown" )
