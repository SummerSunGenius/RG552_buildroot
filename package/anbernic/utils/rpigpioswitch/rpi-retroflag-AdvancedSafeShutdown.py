...
def poweroff():
	while True:
		GPIO.wait_for_edge(powerPin, GPIO.FALLING)
 		output = int(subprocess.check_output(['anbernic-es-swissknife', '--espid']))
 		if output:
...
def reset():
	while True:
		...
		output = int(subprocess.check_output(['anbernic-es-swissknife', '--espid']))
 		output_rc = int(subprocess.check_output(['anbernic-es-swissknife', '--emupid']))
 		if output_rc:
			os.system("anbernic-es-swissknife --emukill")
 		elif output:
			os.system("anbernic-es-swissknife --restart")
		else:
