...
LOCAL_MK	?= $(PROJECT_DIR)/anbernic.mk
EXTRA_PKGS	?=

-include $(LOCAL_MK)

DOCKER_REPO := anberniclinux
IMAGE_NAME  := anbernic.linux-build

TARGETS := $(sort $(shell find $(PROJECT_DIR)/configs/ -name 'b*' | sed -n 's/.*\/anbernic-\(.*\)_defconfig/\1/p'))
UID  := $(shell id -u)
GID  := $(shell id -g)
USER := $(shell whoami)
...
anbernic-docker-image: .ba-docker-image-available

update-docker-image:
	-@rm .ba-docker-image-available > /dev/null
	@$(MAKE) anbernic-docker-image
...
%-clean: anbernic-docker-image output-dir-%
...
%-config: anbernic-docker-image output-dir-%
	@cp -f $(PROJECT_DIR)/configs/anbernic-$*_defconfig $(PROJECT_DIR)/configs/anbernic-$*_defconfig-tmp
	@for opt in $(EXTRA_OPTS); do \
		echo $$opt >> $(PROJECT_DIR)/configs/anbernic-$*_defconfig ; \
	done
	@docker run -it --init --rm \
		-v $(PROJECT_DIR):/build \
		-v $(DL_DIR):/build/buildroot/dl \
		...
		-u $(UID):$(GID) \
		$(DOCKER_REPO)/$(IMAGE_NAME) \
		make O=/$* BR2_EXTERNAL=/build -C /build/buildroot anbernic-$*_defconfig
	@mv -f $(PROJECT_DIR)/configs/anbernic-$*_defconfig-tmp $(PROJECT_DIR)/configs/anbernic-$*_defconfig

%-build: anbernic-docker-image %-config ccache-dir dl-dir
	@docker run -it --rm \
		-v $(PROJECT_DIR):/build \
		-v $(DL_DIR):/build/buildroot/dl \
		-v $(OUTPUT_DIR)/$*:/$* \
...
%-shell: anbernic-docker-image output-dir-%
	@docker run -it --rm \
		-v $(PROJECT_DIR):/build \
		-v $(DL_DIR):/build/buildroot/dl \
		-v $(OUTPUT_DIR)/$*:/$* -w /$* \
...
%-webserver: output-dir-%
	$(if $(wildcard $(OUTPUT_DIR)/$*/images/anbernic/*),,$(error "$* not built!"))
	$(if $(shell which python 2>/dev/null),,$(error "python not found!"))
	python -m http.server --directory $(OUTPUT_DIR)/$*/images/anbernic
...
%-flash: %-supported
	...
	@gzip -dc $(OUTPUT_DIR)/$*/images/anbernic/anbernic-*.img.gz | sudo dd of=$(DEV) bs=5M status=progress
	@sync
...
%-upgrade: %-supported
	...
	-@sudo rm /tmp/mount/boot/anbernic
	@sudo tar xvf $(OUTPUT_DIR)/$*/images/anbernic/boot.tar.xz -C /tmp/mount --no-same-owner
	@sudo umount /tmp/mount
	-@rmdir /tmp/mount
