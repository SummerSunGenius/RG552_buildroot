...
xu4_fusing() {
        ...
        echo "BL1 fusing"
        dd if="${BINARIES_DIR}/bl1.bin.hardkernel"            of="${ANBERNICIMG}" seek=$signed_bl1_position conv=notrunc || return 1

        echo "BL2 fusing"
        dd if="${BINARIES_DIR}/bl2.bin.hardkernel.720k_uboot" of="${ANBERNICIMG}" seek=$bl2_position        conv=notrunc || return 1

        echo "u-boot fusing"
        dd if="${BINARIES_DIR}/u-boot.bin"         of="${ANBERNICIMG}" seek=$uboot_position      conv=notrunc || return 1

        echo "TrustZone S/W fusing"
        dd if="${BINARIES_DIR}/tzsw.bin.hardkernel"           of="${ANBERNICIMG}" seek=$tzsw_position       conv=notrunc || return 1

        echo "u-boot env erase"
        dd if=/dev/zero of="${ANBERNICIMG}" seek=$env_position count=32 bs=512 conv=notrunc || return 1
}
...
(cd "${BINARIES_DIR}/boot" && tar -I "xz -T0" -cf "${ANBERNIC_BINARIES_DIR}/boot.tar.xz" extlinux tools boot anbernic-boot.conf boot-logo.bmp.gz) || exit 1
...
xu4_fusing "${BINARIES_DIR}" "${ANBERNICIMG}" || exit 1
sync || exit 1
