...
ANBERNIC_TARGET=$(grep -E "^BR2_PACKAGE_ANBERNIC_TARGET_[A-Z_0-9]*=y$" "${BR2_CONFIG}" | sed -e s+'^BR2_PACKAGE_ANBERNIC_TARGET_\([A-Z_0-9]*\)=y$'+'\1'+)
...
mkdir -p "${TARGET_DIR}/usr/share/anbernic/datainit/cheats" || exit 1
ln -sf "/userdata/cheats"                                       "${TARGET_DIR}/usr/share/anbernic/datainit/cheats/custom"   || exit 1
...
# make /etc/shadow a file generated from /boot/anbernic-boot.conf for security
rm -f "${TARGET_DIR}/etc/shadow" || exit 1
touch "${TARGET_DIR}/run/anbernic.shadow"
(cd "${TARGET_DIR}/etc" && ln -sf "../run/anbernic.shadow" "shadow") || exit 1
# ln -sf "/run/anbernic.shadow" "${TARGET_DIR}/etc/shadow" || exit 1
...
if test "${ANBERNIC_TARGET}" = "X86" -o "${ANBERNIC_TARGET}" = X86_64
then
...
(cd "${TARGET_DIR}/usr/share/zoneinfo" && find -L . -type f | grep -vE '/right/|/posix/|\.tab|Factory' | sed -e s+'^\./'++ | sort) > "${TARGET_DIR}/usr/share/anbernic/tz"
...
# bios file
python "${BR2_EXTERNAL_ANBERNIC_PATH}/package/anbernic/core/anbernic-scripts/scripts/anbernic-systems" --createReadme > "${TARGET_DIR}/usr/share/anbernic/datainit/bios/readme.txt" || exit 1
