...
cp "${BINARIES_DIR}/rootfs.squashfs" "${BINARIES_DIR}/rpi-firmware/boot/anbernic.update" || exit 1
cp -pr "${BINARIES_DIR}/tools"       "${BINARIES_DIR}/rpi-firmware/"                || exit 1

echo "creating boot.tar.xz"
tar -I "xz -T0" -cf "${ANBERNIC_BINARIES_DIR}/boot.tar.xz" -C "${BINARIES_DIR}/rpi-firmware" "." ||
...
mv "${BINARIES_DIR}/rpi-firmware/boot/anbernic.update" "${BINARIES_DIR}/rpi-firmware/boot/anbernic" || exit 1
